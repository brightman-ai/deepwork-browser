// Package browser 实现 browser Browser Runtime。
// 以 A11y+Element Refs 为核心，为 LLM 提供低 token 高精度 Web 感知通道。
//
// 铁律 : 本包零依赖 Deepwork 上下文（conversation/topic/webui/memory/agent/llm 均不得引入）。
// 铁律 : 使用 chromedp Go 直连 CDP，不引入 Node.js/TS 运行时。
package browser
...
var (
	...
	// ErrPasswordField — 密码字段需要安全输入，不经 LLM/WS 明文通道处理 。
...
type SessionCore interface {
	...
	// SnapWithOptions 获取快照并应用 SnapOptions 过滤（selector/compact/max-depth）。
...
type BrowserCore interface {
	...
	// WS handler 通过 hub.Subscribe(connID) 获取独立帧 channel 。
	...
	// 否则在主标签(browserCtx)执行。[BUG-FIX + ]
	EvalJS(ctx context.Context, expr string, result interface{}) error

	// GetTargetTracker 返回 TargetTracker（多 Target 自动跟随）。
