...
// ============================================================
// § LiveViewEngine [Ref: CAP-BS09-C3, T5-B6]
// ============================================================

// liveViewEngine 实现 Screencast 帧推送（事件驱动 + 即时 ACK）[IR-04]。
...
// StartScreencast 启动 Screencast 帧推送，将帧 fan-out 到 hub [IR-04, TC-09-U-09]。
//
// 关键约束:
//   - 每帧必须立即 ACK（Page.ScreencastFrameAck），不等前端消费 [IR-04, DDC-09]
//   - 帧通过 FrameBroadcastHub.Publish 广播，subscriber channel 满时保新丢旧 [TC-09-U-09]
...
// 关键: 立即 ACK，不可等前端消费 [IR-04, DDC-09, TC-09-U-09]。
// 帧速率限制: 最小间隔 100ms (≈10fps) 防止 burst 压垮 WS [TH-0405-k8r]
...
func (e *liveViewEngine) handleFrameFrom(sourceCtx context.Context, event *page.EventScreencastFrame) {
	...
	// 将帧广播给所有 subscriber（保新丢旧，由 hub 管理）[TC-09-U-09]
	...
	// 立即 ACK — 在 goroutine 中执行，不阻塞当前帧处理 [IR-04]
	e.ackFrame(sourceCtx, event.SessionID)
}
...
// SwitchTarget 切换 Screencast 到新 Target [CAP-BS09-C3 r3]。
...
// PushFrame 推送模拟帧到 hub（测试用，模拟 IR-04 ACK）。
