// Package browser — v2 BrowserPool 测试矩阵.
//
// 绑定 TC (T6 v2):
// - : BrowserPool keying — 同 identity 多 acquire 复用同 entry (单元: 校验错误路径)
// - : BrowserPool Role 校验 — Role 不在 4 枚举 → ErrInvalidRole (无 Chrome 启动)
// - : council Role 必须配合 council-* profile (P1)
// - : Tab 四元身份唯一性 (covered in tab_index_test.go)
// - /48: Pool e2e (需 Chrome — Environment Gate skip 兜底)
// - 辅助: LegacyAcquireTab 行为对齐 r1/r2 ( shim 验证)
...
//
// + : Pool keying 不变式.
// : lazy launch — 第 1 次 Acquire 启动 Chrome, 第 2 次同 key 复用.
//
// 正向断言:
...
// ============================================================
// § : 双阶段退出 bounded wait + audit (CAP §3.5)
// ============================================================
