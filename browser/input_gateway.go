...
// InputGateway 输入网关 — 替代 TakeoverController 
type InputGateway struct {
	...
	recordBuffer *RecordBuffer // 录制缓冲 (nil = 未启用)
}
...
func (g *InputGateway) HandleInput(connID string, msg *InputMessage) *InputAck {
	...
	// Recording tap — 零侵入，仅在 recording 时执行 
...
// Recording tap 在 dispatch 成功 (accepted) 后触发，零侵入主路径 。
