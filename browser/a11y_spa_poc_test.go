...
// 集成级 POC 见 journey_test.go TC-09-J-05 (//go:build integration)。
//
// 目的:
...
func TestA11ySPA_CompactText_SPAElements(t *testing.T) {
	...
	// 验证 compact text 包含关键信息（TH-0405-p7c 新格式: "{N}. role 'name'"）
	checks := []struct {
		contains string
		reason   string
	}{
	...
	if tokenEst >= 2000 {
		t.Errorf("A11ySPA: TokenEst=%d >= 2000 (DDC-06 violation) for %d SPA elements",
...
// TestA11ySPA_ImplementationStatus 记录 BS-09 A11y SPA POC 当前实现状态。
func TestA11ySPA_ImplementationStatus(t *testing.T) {
	...
	status := map[string]string{
		...
		"HA-02 L2 集成验证":       "in integration_test.go TC-09-I-04 (需要 Chrome)",
		"HA-02 L3 Journey POC": "in journey_test.go TC-09-J-05 (需要 Chrome)",
		"动态内容更新":              "每次 snap 完全重建 refTable (GetSnapshot 行为)",
		"iframe 覆盖":            "DEFERRED (TC-09-I-04 备注: 复杂 iframe 场景需进一步验证)",
	}
