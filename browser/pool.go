// Package browser — BrowserPool: identity-keyed Chrome 实例池 [v2 ]
//
// 设计依据:
// - 单 Chrome 多 Tab
// - v2: MERGED — identity-keyed 多 Chrome 多 Tab
//
// 核心架构 (v2 ):
...
// - graceful shutdown: 通过 GracefulShutdown 接口暴露 ( 内部仍 SIGTERM-only
// 双阶段 bounded wait + SIGKILL fallback 由 实装 — 接口已 final)
...
//
// CAP 锚点: (BrowserPool interface) + §3.5/§3.6 + T5 §0.1, §0.3
package browser
...
// ErrInvalidRole — AcquireTabRequest.Role 不在 4 枚举矩阵 .
var ErrInvalidRole = errors.New("browser pool: invalid role (allowed: human/agent/council/background)")

// ErrInvalidRoleIdentity — Role=council 但 IdentityKey 对应 Profile 非 council-* 前缀 (P1).
...
type AcquireTabRequest struct {
	...
	InitialURL string // 可选, 创建 Tab 时直接 navigate ( 暂不实装 navigate)
...
//
// 不变式:
// - 同 IdentityKey 在 BrowserPool.entries 中只有一个 entry ( 守护).
...
type chromePoolEntry struct {
	...
	// [: visible Chrome 必须经 Workspace SSOT 走 D1 序列以绑定到目标 Space]
...
type BrowserPool struct {
	...
	// workspace: visible Chrome 启动 SSOT.
...
func defaultPresetForConfig(c PoolConfig) Preset {
	...
	if preset != nil {
		...
		// Timezone 留空: r1/r2 也未单独追踪; IsolationPolicy 引入显式时区时再补
	}
	return out
}
...
//
// 不变式:
// - 同 IdentityKey 多次 AcquireTab 复用同一 Chrome 实例 .
// - 不同 IdentityKey 的 AcquireTab 启动独立 Chrome 实例 ( 反向断言).
// - Role 不在 4 枚举 → ErrInvalidRole + 不启动 Chrome .
// - Role=council 但 IdentityKey 对应 Profile 非 council-* 前缀 → ErrInvalidRoleIdentity (P1).
// - lazy launch: IdentityKey 未见过 → 调 ChromeLauncher + ProfileManager + IsolationPolicy.Apply .
//
// 校验顺序:
...
//
// 不立即关闭 Chrome — 由 Pool 自身决定 idle eviction ( 实装 idle policy).
...
// ChromePID 当前由 Pool 反查 entry.allocCtx 推断 ( 简化为 entry.started ? -1 : 0
// 真实 PID 提取由 双阶段退出实装时引入 process tracking).
func (p *BrowserPool) Inspect PoolSnapshot {
	...
	for i := range rows {
		if entry, ok := p.entries[rows[i].Key]; ok {
			...
			if entry.started {
				rows[i].ChromePID = -1 // sentinel: started but PID extraction deferred to
			}
		}
	}
	return PoolSnapshot{
...
// GracefulShutdown 双阶段退出 (CAP §2.bis lines 109-113 + §3.5) .
//
// 协议 (per entry):
...
func (p *BrowserPool) GracefulShutdown(ctx context.Context) error {
	...
	for key, entry := range p.entries {
		...
		// 让 Chrome 主动 flush cookie/storage 后退出 (CAP §3.5 持久化 invariant).
		...
		// : Workspace 是 Chrome 进程 SSOT, Pool 通过 ChromeHandle.Kill 释放.
...
//
// 现有 cmd/deepwork/main.go 走此入口; + 调用方应改用 GracefulShutdown(ctx) error.
...
//
// lazy: 该 IdentityKey 第一次见 → 创建 entry + 启动 Chrome .
// 第二次起 → 复用 ( + ).
...
//
// IsolationPolicy.Apply 在 Chrome 启动后通过 BrowserSessionHandle 落地 (: NoopPolicy 直接 nil).
func (p *BrowserPool) startChromeForEntryLocked(ctx context.Context, entry *chromePoolEntry) error {
	...
	// IsolationPolicy.Apply 落地 (: NoopPolicy 直接 nil; ProxyPolicy P1)
...
// ============================================================
var _ = target.SessionID("") // keep target import live (reserved for PID extraction)
