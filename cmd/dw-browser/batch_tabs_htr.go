...
const (
	cliBatchBaseTimeout    = 30 * time.Second
	cliBatchStepTimeout    = 15 * time.Second
...
func runBatch(args []string) {
	...
	positional, flags := parseCommonFlags(clean, "batch")
	if flags.sessionID == "" {
		fmt.Fprintln(os.Stderr, "dw-browser batch: requires --id <id>")
		os.Exit(exitRunErr)
	}
	steps, err := loadBatchSteps(filePath, positional)
	if err != nil {
		fmt.Fprintf(os.Stderr, "dw-browser batch: %v\n", err)
		os.Exit(exitRunErr)
	}
	if len(steps) == 0 {
		fmt.Fprintln(os.Stderr, "dw-browser batch: no actions")
		os.Exit(exitRunErr)
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), cliBatchBaseTimeout+time.Duration(len(steps))*cliBatchStepTimeout)
	defer cancel()
	impl := connectSession(ctx, sessionInfo, "batch", flags)
	defer impl.Close(ctx)
	impl.RestoreRefsFromSession(sessionInfo.Refs)

	results := make([]map[string]interface{}, 0, len(steps))
	for idx, step := range steps {
		observe := forceSnap || step.Snap || needsPostActionSnapshot(step.Action)
		snap, err := impl.ActWithSessionMode(ctx, step.Action, observe)
		if err == nil && step.Await {
			time.Sleep(browser.TargetPostActionDiscoveryDelay)
			if snap2, snapErr := impl.SnapWithSessionMode(ctx, sessionInfo.SnapEpoch+1); snapErr == nil {
				snap = snap2
			}
		}
		...
		if snap != nil {
			...
			sessionInfo.Refs = refsFromSnapshot(snap)
			result["url"] = snap.URL
			result["title"] = snap.PageTitle
			result["refs_count"] = len(snap.Refs)
			injectSnapshotState(result, snap)
			if step.Snap || forceSnap {
				result["summary"] = snap.Text
			}
		}
		results = append(results, result)
	}
	...
	out := map[string]interface{}{
		...
		"batch_run_id":       browser.NewBrowserRunID(sessionInfo.BrowserSessionID+"-batch", sessionInfo.ChromePID),
		"steps":              results,
	}
...
func loadBatchSteps(filePath string, positional []string) ([]batchStep, error) {
	var lines []string
	if filePath != "" {
	...
	steps := make([]batchStep, 0, len(lines))
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "{") {
			if err := json.Unmarshal([]byte(line), &step); err != nil {
				return nil, fmt.Errorf("parse JSONL step %q: %w", line, err)
			}
		} else {
			step.Action = line
		}
		if strings.TrimSpace(step.Action) == "" {
			return nil, fmt.Errorf("batch step missing action")
		}
